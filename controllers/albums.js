...
exports.getAlbum = (req, res) => {
     ...
     if (err) {
          if (err.msg === 'not_found') {
               res.status(404).send({message: `Not found Album with id ${req.params.id}.`,});
          }    
          else {
          res.status(500).send({message: 'Error retrieving Album with id ' + req.params.id,});
          }
     } 
     else res.status(200).json(data);
     });
...
// Validate Request
     ...
     if (err) {
          if (err.msg === 'not_found') {
               ...
               res.status(500).send({message: 'Error updating Album with id ' + req.params.id,});
          }
     } 
     else res.status(200).json(data);
})
...
exports.deleteAlbum = (req, res) => {
     Album.remove(req.params.id, (err,data) => {
          if (err) {
               ...
               else {res.status(500).send({message: 'Could not delete Album with id ' + req.params.id,});
               }
          } 
