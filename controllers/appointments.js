...
exports.getAppointments = async(req,res,next)=>{
     ...
     } else{ //if user == admin can see all
...
exports.addAppointment = async(req,res,next)=>{
     try{
          ...
          //If user not admin,they can create only 3 appointment
          if(existAppointments.length >=3 && req.user.role !== 'admin'){
               return res.status(400).json({success:false,message:`The user with ID ${req.user.id} has 
                    already made 3 appointments` })
          }
...
exports.updateAppointment = async(req,res,next)=>{
     try{
          ...
          if(appointment.user.toString()!== req.user.id && req.user.role !== 'admin'){
               return res.status(401).json({success:false,message:`User ${req.user.id} 
                    is not authorized to update this appointment`})
          }
